...
def packet_catcher(client_socket):
    ...
    while 1:
        ...
        if serverChecksum != bitsum:
            ...
            print_lock.release()
            for close_timer in range(base % ((N + 1) * MSS), NewSeqNum % ((N + 1) * MSS), MSS):
                ...
                if timer_window[NewSeqNum % ((N + 1) * MSS)].running():
                    timer_window[NewSeqNum % ((N + 1) * MSS)].stop()
                    print_lock.acquire()
                    print("Timer #", NewSeqNum % ((N + 1) * MSS), "closedAAAAA")
                    print_lock.release()
            base_lock.acquire()
            ...
            if timer_window[NewSeqNum % ((N + 1) * MSS)].running():
                timer_window[NewSeqNum % ((N + 1) * MSS)].stop()
                print_lock.acquire()
                print("Timer #", NewSeqNum % ((N + 1) * MSS), "closed")
                print_lock.release()
            print_lock.acquire()
...
if __name__ == "__main__":
    ...
    while 1:
        while (base + (N * MSS)) not in packet_Queue and SeqNum < (file_size + initialSeqNum):
        ...
        if nextSeqNum < (base + (N * MSS)) and base < (file_size + initialSeqNum):
            ...
            if nextSeqNum < (file_size + initialSeqNum):
                ...
                timer_window[nextSeqNum % ((N + 1) * MSS)].start()
